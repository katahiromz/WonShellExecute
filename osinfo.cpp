...
BYTE staticIsOS(DWORD dwInfoType)
{
    ...
    case OS_WIN98ORGREATER:
        if (!IsWin9x()) return 0;
        if (g_verInfo.dwMajorVersion > 4) return 1;
    ...
    case OS_WIN98_GOLD:     // 0x06  Win98 RTM (build 1998)
        return IsWin9x()
            && IsVersion(4, 10)
            && LOWORD(g_verInfo.dwBuildNumber) == 1998;

    case OS_WIN2000PRO:     // 0x08
        if (!IsWorkstation()) return 0;
        return IsVersion(5);

    case OS_WIN2000SERVER:  // 0x09
        if (!IsServer()) return 0;
        if (HasSuite(SUITE_ENTERPRISE | 0x02)) return 0; // Advanced Server / Datacenter
        return IsVersion(5);
    ...
    case OS_WIN2000TERMINAL: // 0x0C  Terminal Services
        if (!HasSuite(SUITE_TERMINAL)) return 0;
        return IsVersionOrLater(5);
    ...
    case OS_TERMINALCLIENT: // 0x0E
        return static_cast<char>(GetSystemMetrics(SM_REMOTESESSION));
    ...
    case OS_WIN95_GOLD:     // 0x10  Win95 RTM (build 950)
        return IsWin9x()
            && IsVersion(4, 0)
            && LOWORD(g_verInfo.dwBuildNumber) == 950;
    ...
    case OS_XPORGREATER:    // 0x12
        if (!IsWinNT()) return 0;
        if (g_verInfo.dwMajorVersion > 5) return 1;
        if (g_verInfo.dwMajorVersion < 5) return 0;
        if (g_verInfo.dwMinorVersion)     return 1;
    ...
    case OS_DATACENTER:     // 0x15
        if (g_verInfo.wProductType == VER_NT_SERVER)        goto check_datacenter;
        if (g_verInfo.wProductType != VER_NT_DOMAIN_CONTROLLER) return 0;
check_datacenter:
        return HasSuite(0x40) && IsTSSuiteBasic();

    case OS_ADVSERVER:      // 0x16
        if (g_verInfo.wProductType != VER_NT_SERVER &&
            g_verInfo.wProductType != VER_NT_DOMAIN_CONTROLLER) return 0;
        return HasSuite(SUITE_ENTERPRISE) && IsTSSuiteBasic();

    case OS_SERVER:         // 0x17
        if (!IsServer()) return 0;
        if (HasSuite(0xA2)) return 0; // SUITE_ENTERPRISE | SUITE_BACKOFFICE | 0x02
    ...
    case OS_FASTUSERSWITCHING: // 0x1A
        if (!HasSuite(SUITE_TERMINAL | SUITE_SINGLEUSERTS)) return 0;
    ...
    case OS_WELCOMELOGONUI:  // 0x1B
        if (!IsWorkstation())           return 0;
        if (IsMachineDomainMember())    return 0;
    ...
    case OS_DOMAINMEMBER:   // 0x1C
        return static_cast<char>(IsMachineDomainMember());

    case OS_ANYSERVER:      // 0x1D
        return IsServer();

    case OS_WOW6432:        // 0x1E
        return static_cast<char>(RunningOnWow64());
    ...
    case OS_TABLETPC:       // 0x21
        return static_cast<char>(GetSystemMetrics(SM_TABLETPC));

    case OS_SERVERADMINUI:  // 0x22
        return static_cast<char>(ShouldShowServerAdminUI());

    case OS_MEDIACENTER:    // 0x23
        return static_cast<char>(GetSystemMetrics(SM_MEDIACENTER));

    case OS_APPLIANCE:      // 0x24
        return static_cast<char>(IsApplianceServer());
